...
int main() 
{
	...
	if (imgName == "exemplo1")
	{
		...
		cv::line(final, point1, point2, cv::Scalar(255, 255, 255), 3, cv::LINE_AA);
		cv::line(final, pt21, pt22, cv::Scalar(255, 255, 255), 3, cv::LINE_AA);
	}
	else
	{
		...
		point2.y = img.rows - 50;
		cv::line(final, point1, point2, cv::Scalar(255, 255, 255), 3, cv::LINE_AA);
		cv::line(final, pt11, pt12, cv::Scalar(255, 255, 255), 3, cv::LINE_AA);
	}
	cv::circle(final, cv::Point(x, y), 5, cv::Scalar(0, 0, 255), -1, 32);
	...
	*/
	...
	//exemplo 2
	//float edge = -(_theta +15) * (PI / 180);
	//exemplo 3
	//float edge = -(_theta - 10) * (PI / 180);
	...
	for (int i = -900; i < 900; i++)
	{
		...
		//cv::circle(final, cv::Point(rotation_edge_x - 65, rotation_edge_y - 65), 1, cv::Scalar(0, 255, 255), -1, cv::LINE_AA);
		//cv::circle(final, cv::Point(rotation_edge_x + 170, rotation_edge_y + 120), 1, cv::Scalar(0, 255, 255), -1, cv::LINE_AA);		
		cv::circle(final, cv::Point(rotation_edge_x, rotation_edge_y), 1, cv::Scalar(0, 255, 255), -1, cv::LINE_AA);
	}

	cv::imwrite("D:/VisaoComputacional/Activities/5dc1b836654be800018c3482/images/final_1.png", final);
